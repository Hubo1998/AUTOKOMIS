...
public class Menu {
    ...
    public void showCarBase(){
        ...
        System.out.println("Menu:\n1. Pokaż menu\n2. Kup samochód\n3. Sprawdź samochód");
        int inputshowcar=scanner.nextInt();
        switch (inputshowcar) {
            case 1 -> showFirstPage();
            case 2 -> {
                System.out.println("Wpisz numer samochodu, który chciałbyś kupić:");
                int inputbuycar = scanner.nextInt();
                generate.buyCar(generate.baseOfCars.get(inputbuycar - 1), Player.player1);
            }
            case 3 -> {
                System.out.println("Wpisz numer samochodu, który chciałbyś sprawdzić:");
                int inputcheckcar = scanner.nextInt();
                System.out.println(generate.baseOfCars.get(inputcheckcar-1).producer+" "+generate.baseOfCars.get(inputcheckcar-1).model);
                generate.baseOfCars.get(inputcheckcar-1).carPartsStatus();
            }
        }
    }
    public void checkGarageStatus(Player player){
        ...
        System.out.println("Menu:\n1. Menu\n2. Sprawdź stan auta\n3. Napraw auto\n4. Sprzedaj auto");
        int inputcheckgarage=scanner.nextInt();
        switch (inputcheckgarage){
            case 1->showFirstPage();
            case 2->{
                System.out.println("Wpisz numer samochodu,który chcesz sprawdzić");
                int inputcheckcar=scanner.nextInt();
                System.out.println(player.playerGarage.get(inputcheckcar-1).producer+" "+player.playerGarage.get(inputcheckcar-1).model);
                player.playerGarage.get(inputcheckcar-1).carPartsStatus();
                System.out.println("W takim stanie jest Twoje auto");
            }
            case 3->{
                System.out.println("Które auto chciałbyś naprawić? Wybierz numer auta.");
                int inputcartorepair=scanner.nextInt();
                System.out.println(player.playerGarage.get(inputcartorepair-1).producer+" "+player.playerGarage.get(inputcartorepair-1).model);
                player.playerGarage.get(inputcartorepair-1).carPartsStatus();
                System.out.println("Który podzespół chciałbyś naprawić?");
                int inputparttorepair=scanner.nextInt();
                switch (inputparttorepair) {
                    case 1 -> Mechanicchoice(Components.Comps.brakes, inputcartorepair-1);
                    case 2 -> Mechanicchoice(Components.Comps.suspension, inputcartorepair-1);
                    case 3 -> Mechanicchoice(Components.Comps.engine, inputcartorepair-1);
                    case 4 -> Mechanicchoice(Components.Comps.body, inputcartorepair-1);
                    case 5 -> Mechanicchoice(Components.Comps.gearbox, inputcartorepair-1);
                    default -> {
            ...
            case 4->{
                ...
                int inputcartosell=scanner.nextInt();
                System.out.println(player.playerGarage.get(inputcartosell-1).producer+" "+player.playerGarage.get(inputcartosell-1).model+" za "+player.playerGarage.get(inputcartosell-1).finalPrice);
                System.out.println("Komu chciałbyś sprzedać auto?");
                generate.viewBaseOfClients();
                int inputclienttosell=scanner.nextInt();
                generate.moodOfClient(inputcartosell-1,Player.player1,generate.baseOfClients.get(inputclienttosell-1));
            }
        }
    }
    public void Mechanicchoice(Components.Comps comps,Integer cartorepair){
        ...
        int inputmechanicchoice=scanner.nextInt();
    ...
    public void buyAdvertisement(Player player){
        ...
        int inputAdvertisementchoice=scanner.nextInt();
